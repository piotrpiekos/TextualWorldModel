...
def get_instruction_prompt(env, task="MiniHack-ExploreMaze-Hard-Mapped-v0"):
    if "corridor" in task.lower():
        ...
        goal = "You are playing river, a box-pushing game similar to Sokoban. Your goal is to push the boulders into the river on the east side of the map, walk across the river and get into the special point denoted by \">\" on the other side of the river. The only possible actions are \"west\", \"north\", \"east\" and \"south\", do not output anything else."
    else:
        goal = "Your goal is to get as far as possible in the game."
    import os
    print(os.listdir('.'))
    with open('balrog/world_models/current.json', 'r') as f:
        j = json.load(f)
        wm = j[task]
        # wm = j.get(task, "")
